...
Then('Drag and drop instructions are visible', async ({ page }) => {
  await expect(page.getByLabel('SBOM').getByText('Drag and drop files here or')).toBeVisible();
});
...
Then('Accepted file types are described', async ({ page }) => {
  expect(page.getByLabel('SBOM').getByText('Accepted file types:')).toBeVisible();
});
